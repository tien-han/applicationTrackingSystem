...
async function getFollowUps() {
    ...
    await fetch("/sprint5/data-processing/get-follow-up-applications.php")
        .then(response => {
            if (!response.ok) {
            ...
            data.forEach(application => {
                ...
                if (fiveDaysLate <= follow_up_date && follow_up_date <= fiveDaysAhead) {
                    ...
                    if (today > follow_up_date) {
                        row.innerHTML = `
                            <form id = "${application.applicationsId}" action="/sprint5/form-responses/edit-app-form.php" method = "POST" style="flex-grow: 1; margin-right: 10px;">
