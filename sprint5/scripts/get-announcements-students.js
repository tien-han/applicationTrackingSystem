...
async function getAnnouncementsStudents() {
    ...
    await fetch("/sprint5/data-processing/get-recent-announcements.php")
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            data.forEach(announcements => {
                ...
                if (lastFiveDays <= announcement_date &&
                    ...
                    row.innerHTML =
                        `
                            <a href="/sprint5/form-responses/admin-announcement-details-page.php?id=${announcements.announcementId}" class="announcement-title"><i class="fa-solid fa-bullhorn"></i>
